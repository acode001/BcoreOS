...
>  **BcoreOS不提供操作系统，而是提供一个把您常用的核心系统打包成一个镜像的方案，您每次打包的核心系统镜像为BcoreOS的一个版本**

---

##  🗺️ BcoreOS解决以下问题

1. 资产归档：把应用软件+配套系统打包为系统镜像（一个具体的BocreOS系统版本），可备份，可归档，可部署现场验证
2. 系统自由：使用什么内核、驱动、发行版本，您做主，而不是使用Docker去屏蔽系统差异
3. 无视历史包袱：开发只关心与自己选择系统的兼容性，因为生产环境的系统会被换成开发选择的系统，二进制一致
4. 跨版本升级：不同的BcoreOS版本可切换，切换后网络及数据（非系统部分）继承
5. 批量差分升级：自动对比归档镜像（一个具体的BocreOS系统版本）与目标机器的差异，仅同步差异部分，实现版本迭代快速上线
6. 去除快照线性成本：归档（一个具体的BocreOS系统版本）即快照，可批量恢复，无需每台主机独立快照
